...
class Model:
    def __init__(self, char_list: List[str]) -> None:
        """Инициализация модели: добавление CNN, RNN и CTC и инициализация TF."""
        self.char_list = char_list
        self.snap_ID = 0

        # Использовать нормализацию по пакету или по популяции
        self.is_train = tf.compat.v1.placeholder(tf.bool, name='is_train')

        # Пакет изображений на входе
        self.input_imgs = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None))

        # Настройка CNN, RNN и CTC
        self.setup_cnn()
        self.setup_rnn()
        self.setup_ctc()

        # Настройка оптимизатора для обучения NN
